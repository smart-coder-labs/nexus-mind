use crate::context_fabric_runtime::{
...
pub async fn apply_provenance_migration(
    ...
    Ok(Json(
        serde_json::json!({"schema_version": 59, "status": "applied"}),
    ))
}
...
pub async fn provenance_migration_status(
    ...
    Ok(Json(
        serde_json::json!({"schema_version": 59, "pending": pending}),
    ))
}
...
pub async fn verify_provenance_migration(
    ...
    migrations::verify_context_fabric_provenance(&conn).map_err(fabric_error)?;
    Ok(Json(
        serde_json::json!({"schema_version": 59, "status": "verified"}),
    ))
}

pub async fn sidecar_migration_status(
    State(store): State<SqliteStore>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    let pending = migrations::context_fabric_sidecar_pending(&conn).map_err(fabric_error)?;
    Ok(Json(
        serde_json::json!({"schema_version": 60, "pending": pending}),
    ))
}

pub async fn apply_sidecar_migration(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "settings:write")?;
    migrations::apply_context_fabric_sidecar(&conn).map_err(fabric_error)?;
    migrations::verify_context_fabric_sidecar(&conn).map_err(fabric_error)?;
    Ok(Json(
        serde_json::json!({"schema_version": 60, "status": "applied"}),
    ))
}

pub async fn verify_sidecar_migration(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    ...
    migrations::verify_context_fabric_sidecar(&conn).map_err(fabric_error)?;
    Ok(Json(
        serde_json::json!({"schema_version": 60, "status": "verified"}),
    ))
}

fn viewer(auth: &AuthContext) -> Option<&str> {
...
pub async fn sidecar_rebuild(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
    Json(input): Json<SidecarBuildRequest>,
) -> Result<Json<context_fabric_sidecar::RebuildSummary>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "settings:write")?;
    migrations::verify_context_fabric_sidecar(&conn).map_err(fabric_error)?;
    context_fabric_sidecar::rebuild(&conn, &auth.org_id, viewer(&auth), &input)
        .map(Json)
...
pub async fn sidecar_cancel(
    ...
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "settings:write")?;
    context_fabric_sidecar::cancel_rebuild(&conn, &auth.org_id, &input).map_err(fabric_error)?;
...
pub async fn sidecar_status(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
) -> Result<Json<context_fabric_sidecar::SidecarStatus>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "memory:read")?;
    context_fabric_sidecar::status(&conn, &auth.org_id, None)
        .map(Json)
        .map_err(fabric_error)
}
...
pub async fn sidecar_verify(
    ...
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "settings:write")?;
    context_fabric_sidecar::verify(&conn, &auth.org_id)
        .map(Json)
        .map_err(fabric_error)
}
...
pub async fn sidecar_cleanup(
    ...
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "settings:write")?;
    let deleted = context_fabric_sidecar::cleanup(&conn, &auth.org_id).map_err(fabric_error)?;
    Ok(Json(serde_json::json!({"deleted": deleted})))
}

pub async fn sidecar_shadow(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
    Json(input): Json<SidecarShadowRequest>,
) -> Result<Json<context_fabric_sidecar::SidecarShadowResponse>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "memory:read")?;
    let flag = match input.capability.to_ascii_lowercase().as_str() {
        "bq" => "CONTEXT_FABRIC_BQ_ENABLED",
        "mrl" => "CONTEXT_FABRIC_MRL_ENABLED",
        _ => return Err(verification_error("sidecar_unavailable")),
    };
    if std::env::var(flag).as_deref() != Ok("shadow") {
        return Err(verification_error("sidecar_unavailable"));
    }
    context_fabric_sidecar::shadow(&conn, &auth.org_id, viewer(&auth), &input)
        .map(Json)
        .map_err(|_| verification_error("sidecar_unavailable"))
}
...
pub async fn publish(
    ...
    store
        .context_runtime()
        .invalidate_all(&auth.org_id, "profile_published");
    Ok(Json(result))
}
...
pub async fn rollback(
    ...
    store
        .context_runtime()
        .invalidate_all(&auth.org_id, "generation_rollback");
...
fn resolve_shadow_query(
    ...
    if query.len() != 768 {
        anyhow::bail!("vector_dimension_mismatch");
    }
    Ok(query)
}
...
fn load_shadow_arena(
    ...
    let embeddings =
        crate::db::queries::get_embeddings_for_org_visible(conn, &auth.org_id, viewer)?;
    embeddings
        .into_iter()
        .map(|(id, blob)| {
            let dense = crate::embed::deserialize(&blob);
            if dense.len() != 768 {
                anyhow::bail!("embedding_dimension_mismatch");
            }
            Ok(crate::context_fabric::ShadowVector {
                id,
                tenant_scope: auth.org_id.clone(),
                dense,
                authorized: true,
                fresh: true,
            })
...
fn generation_stamp(conn: &Connection, sql: &str, org_id: &str) -> u64 {
    let value: String = conn
        .query_row(sql, [org_id], |row| row.get(0))
        .unwrap_or_default();
...
fn validate_rollout(input: &RolloutRequest) -> Result<(), &'static str> {
    ...
    if input.profile_id.trim().is_empty() || input.generation.id.trim().is_empty() {
        return Err("missing_rollout_identity");
    }
    if input.manifest_evidence.trim().is_empty() {
        return Err("missing_manifest_evidence");
    }
    if input.run_evidence.trim().is_empty() {
        return Err("missing_run_evidence");
    }
    if input.approval_operator.trim().is_empty() {
        return Err("missing_approval_operator");
    }
    if !input.manifest_evidence.starts_with("manifest:") {
        return Err("invalid_manifest_evidence");
    }
    if !input.run_evidence.starts_with("run:") {
        return Err("invalid_run_evidence");
    }
    Ok(())
}

fn rollout_error(code: &'static str) -> (StatusCode, Json<ApiError>) {
    verification_error(code)
}

pub async fn rollout_shadow(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
    Json(input): Json<RolloutRequest>,
) -> Result<Json<RolloutState>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "settings:write")?;
    validate_rollout(&input).map_err(rollout_error)?;
    let mut state = store.context_runtime().rollout();
    state.shadow_enabled = input.shadow_enabled;
    state.active_profile = Some(input.profile_id);
    state.active_generation = Some(input.generation);
    state.approval_operator = Some(input.approval_operator);
    state.last_manifest_evidence = Some(input.manifest_evidence);
    state.last_run_evidence = Some(input.run_evidence);
    store.context_runtime().set_rollout(state.clone());
    Ok(Json(state))
}

pub async fn rollout_canary(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
    Json(input): Json<RolloutRequest>,
) -> Result<Json<RolloutState>, (StatusCode, Json<ApiError>)> {
    ...
    if !input.shadow_enabled {
        return Err(rollout_error("shadow_gate_required"));
    }
    let mut state = store.context_runtime().rollout();
    state.canary_enabled = input.canary_enabled;
    state.active_lane = CANDIDATE_LANE.into();
    state.active_profile = Some(input.profile_id);
    state.active_generation = Some(input.generation);
    state.approval_operator = Some(input.approval_operator);
    state.last_manifest_evidence = Some(input.manifest_evidence);
...
pub async fn rollout_promote(
    ...
    require_permission(&conn, &auth, None, "settings:write")?;
    validate_rollout(&input).map_err(rollout_error)?;
    if !input.fallback_baseline {
        return Err(verification_error("baseline_fallback_required"));
    }
    if !store.context_runtime().rollout().canary_enabled {
        return Err(verification_error("canary_gate_required"));
    }
    let mut state = store.context_runtime().rollout();
    state.promotion_enabled = true;
    state.baseline_fallback = input.fallback_baseline;
    state.active_lane = CANDIDATE_LANE.into();
    state.active_profile = Some(input.profile_id);
    state.active_generation = Some(input.generation);
    state.approval_operator = Some(input.approval_operator);
...
pub async fn rollout_rollback(
    ...
    runtime.set_rollout(RolloutState::default());
    runtime.invalidate_all(&auth.org_id, "rollout_rollback");
    Ok(Json(runtime.rollout()))
}
...
pub async fn diagnostics(
    ...
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "memory:read")?;
    let active = active_manifest(&conn, &auth.org_id).map_err(fabric_error)?;
    let runtime = store.context_runtime();
    Ok(Json(
        serde_json::json!({"cache": runtime.stats(), "rollout": runtime.rollout(), "active_profile": active.as_ref().map(|m| &m.profile_id), "active_generation": active.map(|m| m.generation), "reason_codes": ["stale_evidence_excluded", "freshness_unknown_timestamp", "baseline_fallback_required"]}),
    ))
}
...
pub async fn lab_shadow(
    State(store): State<SqliteStore>,
    Extension(auth): Extension<AuthContext>,
    Json(input): Json<ShadowInput>,
) -> Result<Json<ShadowResponse>, (StatusCode, Json<ApiError>)> {
    let db = store.conn();
    let conn = db.lock().map_err(|_| verification_error("db_lock"))?;
    require_permission(&conn, &auth, None, "memory:read")?;
    let capability = input.capability.to_ascii_lowercase();
    let flag_name = match capability.as_str() {
        "bq" => "CONTEXT_FABRIC_BQ_ENABLED",
        "mrl" => "CONTEXT_FABRIC_MRL_ENABLED",
        _ => return Err(verification_error("unsupported_shadow_capability")),
    };
    if std::env::var(flag_name).as_deref() != Ok("shadow") {
        return Err(verification_error("capability_flag_not_shadow"));
    }
    let active = active_manifest(&conn, &auth.org_id)
        .map_err(fabric_error)?
        .ok_or_else(|| verification_error("no_active_generation"))?;
    ...
    let policy_generation = generation_stamp(
        &conn,
        "SELECT count(*) || ':' || COALESCE(max(updated_at), '') FROM policies WHERE org_id = ?1",
        &auth.org_id,
    );
    if active.acl_generation != acl_generation
        || active.policy_generation != policy_generation
        ...
        || active.profile_id != input.baseline_manifest.profile_id
        || active.profile_version != input.baseline_manifest.profile_version
        || active.generation != input.baseline_manifest.generation
    {
        return Err(verification_error("active_profile_generation_mismatch"));
    }

    let query = resolve_shadow_query(&store, input.query, input.query_text)
        .map_err(|error| verification_error(&error.to_string()))?;
    let arena =
        load_shadow_arena(&conn, &auth).map_err(|error| verification_error(&error.to_string()))?;
...
pub async fn assemble(
    ...
    let identity = cache_identity(
        ...
        CacheStage::Compile,
        BASELINE_LANE,
        request.profile_id.as_deref().unwrap_or("baseline"),
    );
    ...
    if !response.abstained {
        let _ = runtime.put(
            ...
            runtime.ttl(request.freshness_window_secs),
            true,
        );
    }
    Ok(Json(response))
...
fn verify_compiled_for_caller(
    ...
    let request = compiled_as_request(
        contract_version,
        profile_id,
        profile_version,
        generation,
...
pub async fn verify(
    ...
    if let Err((_, error)) = verify_compiled_for_caller(
        &conn,
        &auth,
        &request.contract_version,
        &request.profile_id,
        ...
        let status = if code == "evidence_not_found" {
            ClaimStatus::Unauthorized
        } else {
    ...
    let units: HashMap<&str, &crate::context_fabric::CandidateEvidence> = request
        .assembled
        .units
        .iter()
    ...
    for claim in &request.claims {
        let Some(unit) = units.get(claim.unit_id.as_str()) else {
            results.push(ClaimVerification {
                id: claim.id.clone(),
                status: ClaimStatus::Unauthorized,
                reason_codes: vec!["claim_unit_not_allowed".into()],
                locator: None,
            });
            continue;
        };
        if claim.locator != unit.locator {
            results.push(ClaimVerification {
                id: claim.id.clone(),
                status: ClaimStatus::Unauthorized,
                reason_codes: vec!["locator_mismatch".into()],
                locator: None,
            });
            ...
            results.push(ClaimVerification {
                ...
                reason_codes: vec!["generation_mismatch".into()],
                locator: None,
            });
        } else if !unit.fresh {
            results.push(ClaimVerification {
                id: claim.id.clone(),
                status: ClaimStatus::Stale,
                reason_codes: vec!["stale_evidence".into()],
                locator: None,
            });
            ...
            results.push(ClaimVerification {
                id: claim.id.clone(),
                status: ClaimStatus::Contradicted,
                reason_codes: vec!["claim_not_supported_by_unit".into()],
                locator: None,
            });
        } else {
            results.push(ClaimVerification {
                ...
                reason_codes: vec!["claim_not_found".into()],
                locator: None,
            });
        }
    }
    let status = if results
        .iter()
        .all(|claim| claim.status == ClaimStatus::Verified)
    {
        ClaimStatus::Verified
    } else if results
        .iter()
        .any(|claim| claim.status == ClaimStatus::Unauthorized)
    {
        ClaimStatus::Unauthorized
    } else if results
        .iter()
        .any(|claim| claim.status == ClaimStatus::Stale)
    {
        ClaimStatus::Stale
    } else if results
        .iter()
        .any(|claim| claim.status == ClaimStatus::Contradicted)
    {
        ClaimStatus::Contradicted
    } else {
...
mod tests {
    ...
    fn shadow_arena_reads_only_authorized_768d_embeddings() {
        ...
        queries::store_embedding(
            &conn,
            &visible_id,
            &crate::embed::serialize(&vec![1.0; 768]),
        )
        .unwrap();
        let hidden_project =
            queries::create_project(&conn, &auth.org_id, "hidden-project", None, None).unwrap();
        let hidden = queries::upsert_memory(
            &conn,
            &auth.org_id,
            &auth.user_id,
            &StoreMemoryRequest {
                project: Some(hidden_project.name),
                tool: "claude".into(),
                content: "hidden".into(),
                tags: None,
                title: None,
        ...
        .unwrap();
        queries::store_embedding(&conn, &hidden.id, &crate::embed::serialize(&vec![2.0; 768]))
            .unwrap();
    ...
    fn shadow_arena_fails_closed_on_invalid_dimension_and_tenant_isolation() {
        ...
        assert_eq!(
            load_shadow_arena(&conn, &auth).unwrap_err().to_string(),
            "embedding_dimension_mismatch"
        );

        let other_org = "other-org".to_string();
        assert!(
            queries::get_embeddings_for_org_visible(&conn, &other_org, None)
                .unwrap()
                .is_empty()
        );
    }
    ...
    fn shadow_query_requires_backend_service_for_text_and_rejects_arbitrary_arena() {
        ...
        assert_eq!(
            resolve_shadow_query(&store, None, Some("query".into()))
                .unwrap_err()
                .to_string(),
            "embedding_service_unavailable"
        );
        assert!(
            serde_json::from_value::<ShadowInput>(serde_json::json!({ "arena": [] }))
                .unwrap_err()
                .to_string()
                .contains("unknown field `arena`")
        );
    }
    ...
    fn rollout_requires_gate_evidence_and_rejects_lab_capabilities() {
        let base = RolloutRequest {
            profile_id: "candidate".into(),
            generation: GenerationRef {
                id: "g2".into(),
                version: 2,
            },
            manifest_evidence: "manifest:sha256:ok".into(),
            run_evidence: "run:ok".into(),
            approval_operator: "operator".into(),
        ...
        assert_eq!(
            validate_rollout(&lab),
            Err("unsupported_rollout_capability")
        );
    }
}
