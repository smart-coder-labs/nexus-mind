...
use nexusmind::{
    ...
    context_fabric::{
        ...
        EvidenceReference, GenerateRequest, GenerationRef, Locator, VerifyRequest,
        CONTRACT_VERSION, DETERMINISTIC_PROVIDER,
    },
...
fn compiled(unit: CandidateEvidence, abstained: bool) -> AssembleResponse {
    AssembleResponse {
        contract_version: CONTRACT_VERSION.into(),
        abstained,
        units: if abstained { vec![] } else { vec![unit] },
        diagnostics: CompileDiagnostics {
            reason_codes: vec![],
            candidate_count: 1,
            selected_count: 1,
            omitted_sources: vec![],
            coverage: vec!["memory".into()],
        },
    }
}
...
fn generate_request(assembled: AssembleResponse, provider: &str, budget: usize) -> GenerateRequest {
    GenerateRequest {
        request_version: Some(nexusmind::context_fabric::GENERATION_REQUEST_VERSION.into()),
        contract_version: CONTRACT_VERSION.into(),
        profile_id: "lab-profile".into(),
        profile_version: 1,
        generation: GenerationRef {
            id: "g1".into(),
            version: 1,
...
async fn http_assemble_resolves_visible_code_chunk_without_client_content() {
    ...
    migrations::run(&conn).unwrap();
    let (org, _admin, _) =
        queries::bootstrap(&conn, "Code Org", "code-org", "a@code.test", "Admin").unwrap();
    let (member, member_key) =
        queries::invite_user(&conn, &org.id, "m@code.test", "Member", "member").unwrap();
    ...
    let chunk_id = queries::insert_code_chunk(
        &conn,
        project_id,
        "src/lib.rs",
        "sha-code",
...
async fn generation_is_versioned_local_only_and_budgeted() {
    ...
    let (status, body) = post_json(
        ...
        &key,
        "/v1/context/generate",
        serde_json::to_value(generate_request(assembled.clone(), "unknown-provider", 20)).unwrap(),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["abstained"], true);
    assert!(body["metadata"]["reason_codes"]
        .as_array()
        .unwrap()
        .iter()
        .any(|r| r == "provider_unavailable"));
    assert_eq!(body["failure"], "unsupported_provider");

    let (app, key, id, _, _) = http_setup();
    let budget_request = assemble_request(
        &id,
        "backend verified content",
        "memory",
        "memory-search",
    ...
    let (status, body) = post_json(
        ...
        serde_json::to_value(generate_request(
            compiled(budget_request.candidates[0].clone(), false),
    ...
    assert!(body["metadata"]["reason_codes"]
        ...
        .unwrap()
        .iter()
        .any(|r| r == "budget_exceeded"));
    assert_eq!(body["failure"], "budget_exceeded");
}

#[tokio::test]
async fn generation_records_separate_provenance_and_deterministic_run_metadata() {
    let (app, key, id, _, _) = http_setup();
    let candidate = assemble_request(
        &id,
        "backend verified content",
        "memory",
        "memory-search",
    ...
    .candidates[0]
        .clone();
    let request = generate_request(compiled(candidate, false), DETERMINISTIC_PROVIDER, 20);
    ...
    assert_eq!(first["response_version"], "context-generation.response.v1");
    assert_eq!(first["run"]["run_id"], second["run"]["run_id"]);
    assert_eq!(first["retrieval"]["generation"]["id"], "g1");
    assert_eq!(first["compile"]["selected_count"], 1);
    assert_eq!(first["provenance"][0]["locator"]["id"], id);
    assert_eq!(first["output"], first["claims"][0]["text"]);
}

#[tokio::test]
async fn generation_abstains_for_timeout_and_output_overflow() {
    let (app, key, id, _, _) = http_setup();
    let candidate = assemble_request(
        &id,
        "backend verified content",
    ...
    .candidates[0]
        .clone();
    let mut timeout = generate_request(
        compiled(candidate.clone(), false),
        DETERMINISTIC_PROVIDER,
        20,
    );
    timeout.timeout_ms = Some(0);
    let (status, body) = post_json(
        app,
        &key,
        "/v1/context/generate",
    ...
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["failure"], "timeout");
    ...
    .candidates[0]
        .clone();
    let mut overflow = generate_request(compiled(candidate, false), DETERMINISTIC_PROVIDER, 20);
    overflow.output_byte_budget = Some(1);
    let (status, body) = post_json(
        app,
        &key,
        "/v1/context/generate",
    ...
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["failure"], "output_overflow");
}

#[tokio::test]
async fn legacy_generation_request_defaults_to_versioned_response() {
    let (app, key, id, _, _) = http_setup();
    let candidate = assemble_request(
        &id,
        "backend verified content",
    ...
    .candidates[0]
        .clone();
    let mut request = generate_request(compiled(candidate, false), DETERMINISTIC_PROVIDER, 20);
    request.request_version = None;
    let (status, body) = post_json(
        app,
        &key,
        "/v1/context/generate",
    ...
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["response_version"], "context-generation.response.v1");
    assert_eq!(
        body["run"]["request_version"],
        "context-generation.request.v1"
    );
}
...
async fn generation_rejects_uncompiled_context_and_verify_fails_closed() {
    ...
    let (status, body) = post_json(
        ...
        "/v1/context/generate",
        serde_json::to_value(generate_request(
            compiled(request.candidates[0].clone(), true),
            DETERMINISTIC_PROVIDER,
            20,
        ))
        .unwrap(),
    ...
    assert!(body["metadata"]["reason_codes"]
        ...
        .unwrap()
        .iter()
        .any(|r| r == "context_not_compiled"));

    let (verify_app, verify_key, _, secret_id, _) = http_setup();
    let mut unauthorized = assemble_request(
        &secret_id,
        "secret backend content",
        "memory",
        "memory-search",
    ...
    .candidates[0]
        .clone();
    unauthorized.generation = GenerationRef {
        id: "g1".into(),
        version: 1,
    };
    let verify = VerifyRequest {
        ...
        model: "lab-model".into(),
        provider: DETERMINISTIC_PROVIDER.into(),
        assembled: compiled(unauthorized.clone(), false),
        output: Some(unauthorized.content.clone()),
        claims: vec![Claim {
            id: "c1".into(),
            text: unauthorized.content.clone(),
            unit_id: unauthorized.unit_id.clone(),
            locator: unauthorized.locator.clone(),
        }],
    };
    let (status, body) = post_json(
        verify_app,
...
async fn verify_reports_supported_and_unsupported_claims_without_evidence_leak() {
    ...
    let base = VerifyRequest {
        ...
        model: "lab-model".into(),
        provider: DETERMINISTIC_PROVIDER.into(),
        assembled: compiled(candidate.clone(), false),
        output: Some(candidate.content.clone()),
        claims: vec![Claim {
            id: "supported".into(),
            text: "verified".into(),
    ...
    .candidates[0]
        .clone();
    let mut unsupported = VerifyRequest {
        contract_version: CONTRACT_VERSION.into(),
        profile_id: "lab-profile".into(),
        profile_version: 1,
        generation: GenerationRef {
    ...
    let (status, body) = post_json(
        ...
        &key,
        "/v1/context/verify",
        serde_json::to_value(&mut unsupported).unwrap(),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
...
fn memory_write_gate_is_atomic_and_expired_rows_are_not_retrievable() {
    ...
    migrations::run(&conn).unwrap();
    let (org, user, _) =
        queries::bootstrap(&conn, "Gate Org", "gate-org", "gate@test", "Admin").unwrap();
    ...
    .unwrap();
    conn.execute(
        "UPDATE memories SET delete_after = datetime('now', '-1 day') WHERE id = ?1",
        [&memory.id],
    )
    .unwrap();
    assert!(queries::list_memories(
...
fn provenance_roundtrip_is_allow_listed_untrusted_and_tenant_scoped() {
    ...
    let (org, user, _) =
        queries::bootstrap(&conn, "Provenance Org", "provenance-org", "p@test", "Admin").unwrap();
    ...
    let memory = queries::upsert_memory(
        ...
        &nexusmind::models::types::StoreMemoryRequest {
            project: None,
            tool: "claude".into(),
            content: "provenance content".into(),
            tags: None,
            title: None,
            memory_type: None,
            scope: None,
    ...
    .unwrap();
    assert_eq!(
        memory.context_fabric_metadata.as_ref().unwrap().source_id,
        "source-1"
    );
    ...
    let raw: String = conn
        .query_row(
            "SELECT context_fabric_metadata FROM memories WHERE id = ?1",
            [&memory.id],
            |row| row.get(0),
        )
        .unwrap();
    ...
    assert!(
        queries::get_memory_by_id_for_org(&conn, "wrong-org", &memory.id)
            .unwrap()
            .is_none()
    );
    ...
    let before: i64 = conn
        ...
        .unwrap();
    let result = queries::upsert_memory(
        &conn,
        &org.id,
        &user.id,
        &nexusmind::models::types::StoreMemoryRequest {
...
fn archive_restore_lifecycle_revalidates_visibility_and_retention() {
    ...
    let req = nexusmind::models::types::StoreMemoryRequest {
        project: None,
        tool: "test".into(),
        content: "lifecycle content".into(),
        tags: None,
        title: None,
        memory_type: None,
        scope: None,
