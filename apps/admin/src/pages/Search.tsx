...
function MemoryCard({ memory }: { memory: Memory }) {
  return (
    <div className="bg-[#272729] rounded-[11px] border border-border-primary p-4 space-y-2">
      ...
      {/* Content preview */}
      <p className="text-[10px] text-text-secondary leading-relaxed line-clamp-2">{memory.content}</p>
      {/* Footer */}
...
function ProjectCard({ project }: { project: Project }) {
  return (
    <div className="bg-[#272729] rounded-[11px] border border-border-primary p-4 space-y-1.5">
      ...
      {project.description && (
        <p className="text-[10px] text-text-secondary line-clamp-2">{project.description}</p>
      )}
    ...
    <div className="p-8 max-w-3xl mx-auto space-y-6">
          ...
          {TABS.map(({ key, label }) => (
            <button
              ...
              className={cn(
                'transition-colors',
                activeTab === key
                  ? 'bg-accent-blue/10 text-accent-blue rounded-[5px] text-[10px] px-2 py-0.5'
                  : 'text-text-quaternary hover:text-text-secondary rounded-[5px] text-[10px] px-2 py-0.5',
              )}
            >
              {label}
      ...
      {loading && (
        <div className="space-y-3">
          {[0, 1, 2].map(i => (
            <div key={i} className="h-20 animate-pulse bg-white/[0.04] rounded-[8px]" />
          ))}
        </div>
      )}
