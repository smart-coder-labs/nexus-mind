...
| A2 | nm-ui-impl-a | `Agents.tsx`/`ApiKeys.tsx` relative-time bug, `Users.tsx` cleanup, `Dashboard.tsx` focus-ring token swap + **addendum**: neutral stat tiles (§5) + Getting Started strikethrough contrast + 390px string-tile truncation + convert Dashboard FOCUS_CANVAS/FOCUS_TILE from Tailwind `ring` to CSS `outline` per §6 | **complete** (2026-07-03). Agents.tsx + ApiKeys.tsx: UTC toDate() fix + full retype. Users.tsx: statusBadge() refactor (eliminates duplicate status render), native select → B2 Select, type scale corrected. Dashboard: ring→outline, tabular-nums truncate, BADGE_ACCENT tints removed (all stat tiles neutral per §5). |
...
| A3 | nm-ui-impl-a | `/conventions` mobile/header (#13), primary-CTA rule (#15) on Collections + Memories | **complete** (2026-07-03). Conventions: §4 header skeleton, flex-wrap for 390px, Action Blue CTA, inline-hex→token, shadow→border elevation, proper EmptyState. Collections + Memories: ghost→Action Blue pill on primary action only. |
...
| B5 | nm-ui-impl-b | membership-filtering empty state (#24): `CommandPalette.tsx` global search | **complete** (2026-07-03). EMPTY_RESULT sentinel, per-domain null-guard, accessible empty state with membership-scoped copy. |
