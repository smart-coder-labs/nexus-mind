...
## 0. FREEZE (2026-07-03, from main — full test suite + push in progress)

No new batches dispatched until main confirms the push is done. Implementers told to finish or safely pause current work without leaving files half-written, then report tree-stable status.

- nm-ui-impl-b: **STABLE**, confirmed 2026-07-03 — `tsc -p tsconfig.app.json --noEmit` clean, admin+landing build green, no half-written files. B3.1 was already complete before the freeze (not left mid-edit). Only B4 remains not-started (correctly holding).
- nm-ui-impl-a: **STABLE**, confirmed 2026-07-03 — `tsc -p tsconfig.app.json --noEmit` clean (exit 0), page tests 21/21 pass, no half-written files. Dashboard.tsx focus-ring swap + full Agents.tsx/ApiKeys.tsx A2 pass done; Users.tsx untouched (zero edits) and Dashboard addenda (tint/contrast/truncation/ring-outline) not started — both correctly held for after the freeze.
- nm-ux-qa: holding all verification (B1 drawer re-verify + B3 landing re-verify with cache clear both staged, ready to run once freeze lifts); not touching any dev server.

**ALL CLEAR — tree stable, safe for main's test+push (2026-07-03).**
...
| B3 | nm-ui-impl-b | CRITICAL landing repair (#21) + VT323 + reduced motion | **complete** (task #5 done; QA verified end-to-end 2026-07-03: waitlist POSTs to Supabase 201 + success toast, chips show selected state, VT323 removed, reduced-motion guard added, react/react-dom pinned ^19). Note: impl-b's deletion of `apps/landing/pnpm-lock.yaml` as B3 hygiene was flagged by main pre-push and restored from HEAD — `apps/landing` diff is now scoped to just `astro.config.mjs` + `apple-ds.css` |
| B3.1 | nm-ui-impl-b | `Layout.tsx` drawer a11y from QA's B1 pass: `inert` on the mobile `<aside>` tied to `drawerOpen`; Escape closes the open drawer first in the global keydown (Layout.tsx:451); focus management (open→close button, Escape→hamburger) | **complete** — impl-b confirms this was finished before the freeze landed (`inert`+`aria-hidden`, dedicated `[drawerOpen]` Escape effect, `menuButtonRef`, bonus skip-to-content link); tsc+build green. QA re-verify at 390px HELD until freeze lifts (§0) |
...
| B4 | nm-ui-impl-b | backoffice cleanup (#8, #23) | queued |
