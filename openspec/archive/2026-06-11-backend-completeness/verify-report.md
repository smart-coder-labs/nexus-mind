...
2. **`idx_audit_logs_org_ts` uses `timestamp` column**: `audit_logs` uses `timestamp` as the column name, not `created_at`. Correcting it to match the actual schema is ACCEPTABLE.

3. **`ExternalAuditRequest` fields declared `Option<String>` for 400 vs 422 control**: Manual validation in the handler returns 400 correctly. ACCEPTABLE — the observable HTTP behavior matches the spec.
...
**Risk assessment**: Low. The ALTER statements are additive (nullable columns with no defaults required), and the idempotent ignore-on-duplicate pattern handles the partial state.
...
`lookup_plan` returns `"free"` on any error. This fail-open behavior is sensible for availability but could silently downgrade enterprise/team keys to free-tier quotas under DB contention. A tracing warn log on error would aid observability.

### SUGGESTION-02: `sha2` dependency is a direct dep but was not in the original T-01 scope

T-01 only specified `dashmap = "5"`. `sha2 = "0.10"` and `hex = "0.4"` appear to have been present before this change. No action needed — confirming this is not a gap.
...
|------|--------|----------|
| T-01: dashmap dep | Complete | `Cargo.toml` line 39: `dashmap = "5"` |
| T-02: run_v9 migration | Complete | `migrations.rs` lines 418-458; 5 tests |
| T-03: store_memory removed | Complete | No `pub fn store_memory` in src/ or tests/ |
| T-04: AuditEntry hash fields | Complete | `types.rs` has `previous_hash`/`current_hash` with `#[serde(default)]` |
| T-05: GET /v1/memory/:id | Complete | Route registered; handler and query exist |
| T-06: GET /v1/context/project/:project | Complete | Route registered; `context.rs` and `get_project_context` query exist |
| T-07: insert_audit_log_chained | Complete | `queries.rs` line 605; SHA-256 chain implemented |
| T-08: POST /v1/audit/log | Complete | Route registered; `post_audit` handler exists; all 8 tests pass |
| T-09: rate_limit.rs module | Complete | Module exists with all required types and functions |
| T-10: rate_limit wired in router | Complete | `router.rs` lines 58, 87-88 |
...
**Next recommended phase**: `sdd-archive` ✓ (now complete)
